...
var DateProcessing = class {
   ...
   */
  static formatDateForTodoist(date) {
  ...
  static processMomentFormat(dateStr) {
    var _a;
    if (!((_a = this.settings) == null ? void 0 : _a.momentFormatCleanupPatterns)) {
      return null;
    }
...
var RegexPatterns = class {
   ...
   */
  static createEmojiCleanupPattern(emoji) {
...
var URILinkProcessing = class {
  ...
  async generateAdvancedUriToBlock(blockId, editor) {
    ...
    if (!file)
      return "";
    const advancedUriPlugin = (_a = this.app.plugins) == null ? void 0 : _a.getPlugin(
...
var UIDProcessing = class {
  ...
  async ensureUidInFrontmatter(file, editor) {
    var _a;
    const advancedUriPlugin = (_a = this.app.plugins) == null ? void 0 : _a.getPlugin(
      "obsidian-advanced-uri"
    );
  ...
  async getOrCreateUid(file, editor) {
...
var TaskToTodoistModal = class extends import_obsidian6.Modal {
  ...
  onOpen() {
    ...
    const loadProjects = async () => {
      var _a;
      try {
        const projects = await ((_a = this.plugin.todoistApi) == null ? void 0 : _a.getProjects());
        if (projects) {
          projectSelect.empty();
          projects.forEach((project) => {
...
var TODOIST_CONSTANTS = {
  ...
  LINK_TEXT: "\u{1F517} View in Todoist",
  ...
  FORMAT_STRINGS: {
    ...
    TODOIST_LINK: (indentation, linkText, url, timestamp) => `
...
var TodoistTaskSync = class {
  constructor(app, settings, todoistApi, checkAdvancedUriPlugin, URILinkProcessing2, UIDProcessing2, plugin) {
    this.app = app;
    this.settings = settings;
  ...
  async isSharedProject(projectId) {
    var _a, _b;
    try {
      const project = await ((_a = this.todoistApi) == null ? void 0 : _a.getProject(projectId));
      return project ? (_b = project.is_shared) != null ? _b : false : false;
    } catch (error) {
  ...
  async syncSelectedTaskToTodoist(editor) {
    ...
    if (!this.todoistApi) {
      new import_obsidian7.Notice("Please set up your Todoist API token in settings");
      return;
    }
    ...
    try {
      ...
      if (!taskDetails.cleanText) {
        new import_obsidian7.Notice("Task text is empty");
        return;
      }
      new TaskToTodoistModal(
        ...
        taskDetails.dueDate || "",
        ((_a = taskDetails.priority) == null ? void 0 : _a.toString()) || this.settings.todoistDefaultPriority.toString(),
        async (title, description, dueDate, priority, projectId) => {
          try {
            ...
            if (this.todoistApi) {
              ...
              new import_obsidian7.Notice("Task successfully synced to Todoist!");
            } else {
              new import_obsidian7.Notice(
  ...
  async createTodoistTaskFromSelectedText(editor) {
    ...
    try {
      ...
      if (!this.todoistApi) {
        new import_obsidian7.Notice("Please set up your Todoist API token first.");
      ...
      if (!blockId) {
        new import_obsidian7.Notice("Failed to generate block ID.");
        editor.setCursor(currentCursor2);
        return;
      }
      const advancedUri = await this.URILinkProcessing.generateAdvancedUriToBlock(
      ...
      new NonTaskToTodoistModal(
        ...
        async (title, description, dueDate, priority, projectId) => {
          try {
            ...
            new import_obsidian7.Notice("Task successfully created in Todoist!");
          } catch (error) {
            console.error("Failed to create Todoist task:", error);
            new import_obsidian7.Notice(
      ...
      new import_obsidian7.Notice("An error occurred. Please try again.");
      editor.setCursor(currentCursor);
    }
  }
  async createTodoistTaskFromSelectedFile() {
    try {
      ...
      const file = this.app.workspace.getActiveFile();
      if (!file) {
        new import_obsidian7.Notice("No active file found");
        return;
      }
    ...
    try {
      ...
      if (localTaskId) {
        try {
          ...
          return {
            task_id: localTaskId,
            is_completed: (_b = (_a = task.checked) != null ? _a : task.isCompleted) != null ? _b : false
          };
        } catch (error) {
          console.log(
      ...
      if (matchingTask) {
        return {
          ...
          is_completed: (_d = (_c = matchingTask.checked) != null ? _c : matchingTask.isCompleted) != null ? _d : false
        };
      }
      return null;
    } catch (error) {
  ...
  async insertTodoistLink(editor, line, taskUrl, isListItem) {
    const currentCursor = editor.getCursor();
    ...
    try {
      ...
      new import_obsidian7.Notice("Failed to insert Todoist link. Please try again.");
      editor.setCursor(currentCursor);
    }
  }
  /**
  ...
  async syncTodoistDescriptionToObsidian(editor, excludeMetadata = true) {
    ...
    try {
      ...
      if (!todoistTaskId) {
        new import_obsidian7.Notice("No linked Todoist task found for this task.");
        return;
      }
      ...
      const isTaskCompleted = (_b = (_a = task.checked) != null ? _a : task.isCompleted) != null ? _b : false;
      if (isTaskCompleted) {
        new import_obsidian7.Notice("This task is already completed in Todoist.");
        return;
      }
      const description = task.description || "";
      if (!description.trim()) {
        new import_obsidian7.Notice("The task description is completely empty.");
        return;
      }
      const lines = description.split("\n");
      const hasOnlyMetadata = lines.every(
        (line) => !line.trim() || TODOIST_CONSTANTS.METADATA_PATTERNS.ORIGINAL_TASK_PATTERN.test(
          line
      ...
      if (excludeMetadata) {
        filteredLines = lines.filter(
          (line) => !TODOIST_CONSTANTS.METADATA_PATTERNS.ORIGINAL_TASK_PATTERN.test(
            line
          ) && !TODOIST_CONSTANTS.METADATA_PATTERNS.REFERENCE_PATTERN.test(
            line
          ) && line.trim() !== ""
...
${formattedDescription}`,
      ...
      new import_obsidian7.Notice("Failed to sync Todoist description. Please try again.");
    }
  }
  /**
...
// src/main.ts
var TodoistContextBridgePlugin = class extends import_obsidian8.Plugin {
  constructor() {
    super(...arguments);
  ...
  async loadProjects() {
    var _a;
    try {
      const projects = await ((_a = this.todoistApi) == null ? void 0 : _a.getProjects());
      if (projects) {
      }
    } catch (error) {
  ...
  initializeTodoistApi() {
    if (this.settings.todoistAPIToken) {
      this.todoistApi = new import_todoist_api_typescript.TodoistApi(this.settings.todoistAPIToken);
    } else {
      this.todoistApi = null;
    }
  }
  ...
  async verifyTodoistToken(token) {
    try {
      const tempApi = new import_todoist_api_typescript.TodoistApi(token);
